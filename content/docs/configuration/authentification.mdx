...
The only environment variable that is mandatory is the `NEXTAUTH_SECRET`. This is a random value used by the library to encrypt tokens and email verification hashes. You can generate one via running:
